...
% and Tenzin (both acting as driver and observer at various points)
    ...
    fprintf('To enter a number, type "play".\n');
    ...
    while true % infinite loop until user types "exit"
        ...
        % prompts user with the option to enter solution, get a hint or quit the game (if they so choose)
        user_input = input('Type "play" to enter a solution, "hint" to reveal one number in the grid or "solve it for me" to reveal the completed grid. Alternatively, type "exit" to quit the game: ', 's'); 
            ...
            if row >= 1 && row <= 9 && col >= 1 && col <= 9 && num >= 1 && num <= 9 % defines range of valid row and column numbers (1-9)
                if sudoku_grid(row, col) == 0  % checks for empty cell
                    if is_valid(sudoku_grid, row, col, num) % checks for valid move
                        ...
                        disp('Invalid move! That number violates Sudoku rules.');
                    end 
                    else
                        disp('That cell is already filled! Choose an empty cell.');
                end
            else
                disp('Invalid input. Please enter numbers between 1-9.');
            end
        else
            disp('Invalid input. Please type "play", "hint", "solve it for me", or "exit".');
        end
    end 
